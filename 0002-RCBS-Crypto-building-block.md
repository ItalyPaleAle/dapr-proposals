...
- Symmetric keys:
    ...
    - AES-GCM: `AES-GCM`  
      Uses a random nonce. The result of using AES-GCM is in the format `nonce || wfk || tag`  
- RSA keys:
...
> Dapr will automatically choose AES-GCM if running on a system that supports AES-NI CPU instructions, or ChaCha20-Poly1305 otherwise. We may change that behavior in the future. We may also offer a toggle so users can manually select what cipher to use.
